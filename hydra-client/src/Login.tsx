...
const ory = new FrontendApi(
  new Configuration({
    basePath: window.location.origin + "/.ory/kratos",
    baseOptions: {
    ...
    ory
      ...
      .catch((err) => {
        ...
        if (err.response?.status === 400 && err.response?.data?.error?.id === "session_already_available") {
          if (loginChallenge) {
            window.location.href = window.location.origin + `/api/login?login_challenge=${loginChallenge}`;
          } else {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    ory
      ...
      .then(() => {
        ...
        if (challenge) {
          window.location.href = window.location.origin + `/api/login?login_challenge=${challenge}`;
        } else {
          window.location.href = "/";
        }
      })
